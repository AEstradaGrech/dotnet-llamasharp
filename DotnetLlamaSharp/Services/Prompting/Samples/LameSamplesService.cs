    ...
    public class LameSamplesService : ILameSamplesService
    {
        ...
        public async Task<ChatPrompt> TemplatedExamples(ChainedPrompt request)
        {
            ...
            var finalStep = await LameChain
                .StartWith(
                    new StepSettings(
                        _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                            ...
                            request.Settings),
                        new PromptCommandRequest("How does a RAG application work?")
                        ), //this should be always the request.UserPrompt. This is persisted along the chain and passed as context to all the steps to give the context about the goal of the whole chain                   
                    ...
                    chainIntent: "Enhanced Question-Answer chat" // You can use this to keep the consistency of the overall chain context so all the steps know what are they doing in general terms. Leave it empty if you don't need it
                )
                .Then(new StepSettings(
                    _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                        instruction: "Explain the previous output in less than 100 words.",
                        settings: request.Settings), // You can leave the user prompt empty and try with the instruction only, but if you are using local models you may need to enforce the instruction with a more specific user prompt
                ...
                .Then( // ...and this is the most simple usage (only command + default chain user prompt) with an optional feed forward message to guide the next one
                     new StepSettings(
                        ...
                        feedFwd: "Use the provided topic information and explain it to the user according to your role / character" // You can leave this empty (it is not required),
                ...
                .Then(
                     new StepSettings(_factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                        ...
                        feedFwd: "") // You can use the feed forward message of the last step to guide the step that generates the final 'user-friendly' message.
                )
            ...
            var chainResult = await LameChain
                .StartWith(
                    new StepSettings(
                         _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(instruction: "Analyze the user input and extract the intent. Write a brief explanation of what is the user querying about"),
                ...
                .Tap([
                    ...
                    new StepSettings(
                        _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(instruction: "Answer the user query in general terms. Dont'be too specific, instead try to give a global overview of the topic the user is querying about and mention other relevant / related topics"),
                        ...
                        new PromptCommandRequest(request.Prompt, model:"your-fine-tuned-secret-model")
                    ),
                    new StepSettings(
                        ...
                        new VectorSearchRequest( // Simply pass the proper request type for the command with its own parameters etc
                ...
                .Join(new StepSettings(
                    _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                        ...
                        new PromptCommandRequest("Generate a report with a main section and a 'Related Topics of Interest' as instructed"), // enforce the instruction (imagine you are getting 'almost-fine' results with a dumb model... this can help to fix that)
            ...
            chainResult = await LameChain
               ...
               // This splits the chain exactly like in the previous example, but now I'm going to continue chaining commands to each tap command output to keep processing the results separately piping commands on each subchain
                ...
                .Pipe( // Pipe will execute the instruction command over each output from the prevous step, producing an equal number of outputs until you .Join() the subchains
                    ...
                    feedFwd: "Review all the sources and get a consistent overview of the expected character profile.")
                )
            ...
            chainResult = await LameChain
                ...
                .SplitThrough(
                    new StepSettings(
                        ...
                        new PromptCommandRequest($"- USER INPUT: {request.Prompt}"),
            ...
            chainResult = await LameChain
                .StartWith(new StepSettings(_factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                        instruction: "Analyze the user input and extract the intent. Write a brief explanation of what is the user querying about"
                    ),
                    new PromptCommandRequest(request.Prompt)
                ),
                ...
                .SplitThrough(
                    new StepSettings(
                        _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                            instruction: "Use the analyzed 'User Intent' from the previous output along with the user input to write 3 variants of the user question that might enhance the expected response in terms of content detail",
                            request.Settings),
                        requestPrompt: ($"- USER INPUT: {request.Prompt}"),
                        feedFwd: "You can add a guidance message here FROM the Splitted Command TO each plugged instruction / command"),
                    [
                        new StepSettings(
                            _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                ...
                .Pipe( // Pipe will execute the instruction command over each output from the prevous step, producing an equal number of outputs until you .Join() the subchains
                    new StepSettings(
                        _factory.GetAsJsoneable<MessagePromptCommand, ChatMessage>(
                            instruction: "Use the previous sources and the user input to generate a short text (100-150 word) about possible related topics of interest that could lead to some user follow-up question.",
                        request.Settings),
                        requestPrompt: $"- USER QUERY: {request.Prompt}",
                        feedFwd: "Use this generated topics of interest to enhance your final response by adding some extra information for the user to query about"
                    )
                )
                ...
                .ChainFeedsFrom([firstId, splitId]) // Feed the junction step with the outputs from previous SingleThrow / Splitted steps
                .ThenExecuteAsync(withFinalMessage: true, withReplay: true);
        ...
        /// <summary>
        /// Demonstrates how you can add parallel subchains of variable step length using a Tap
        /// </summary>
        /// <param name="request"></param>
        /// <returns></returns>
        ...
        public async Task<ChainResult> StashExample(ChainedPrompt request)
            => await LameChain
                .StartWith(new StepSettings(
                        ...
                        feedFwd: "Use the definition of the intent to understand better what is the user trying to do and use the information in your deliverances",
                        request.Prompt), // pass the prompt to analyze), // enforce the analysis of the intent in the scored bool command to help the LLM decide if it is necesary to retrieve data from previous chats to answer the user
                ...
                .StashIf(
                    new StepSettings(_factory.GetCommand<ScoredBoolCommand, ScoredBoolResponse>(
                        systemMessage: @"Analyze the user input and any other provided support data and determine if is necessary to retrieve previous chat conversations that the system has stored. 
    Note if the user is making references to past conversations with you or events of which you are supposed to be aware of. Check if the user is speaking in past or present, including or mentioning you or 
    speaking in a way that makes you think that you should be aware of some information that is not present in the input"),
                       feedFwd: null,
                       requestPrompt: $"Analyze the user input and reason if it would be necessary to review previous conversations to answer the user or it not. User Input: {request.Prompt}"), // This would not be necessary because the instruction is clear, but it is usefult for local dumb models
                    new StashSettings(
                        _factory.GetSourceable<VectorSearchSourceable>(),
                        new VectorSearchRequest(
                    ...
                    isIsolated: true)
                 ) // Isolated = the stash WON'T read the previous output as the other steps do (this is because you don't want to add noise to the context of the sourceable command or because you just don't need it, like in a VectorSearchSourceable that requires only the user Prompt
                .Then(new StepSettings(
                    _factory.GetMessagePromptCommand(systemMessage: "You are a helpful assistant. Chat with the user in a natural way and use any provided data about previous conversations to offer a more contextualized response to the user"), // settings here are the same that came in the request and passed as Default 
                    new PromptCommandRequest(request.Prompt))) // pass the user input to the final chat step, that will answer to any "new topic" prompt without checking the chat history or perform a DB search to retrieve data to add it to the context in the final step
        ...
        public async Task<ChainResult> ConditionalChatChain(CommandChatRequest request)
            => await LameChain
                ...
                .ThenIf(() => request.ChatHistory.Count > 2, // This is just a dummy condition to trigger the subchain (and the subchain is just a dummy chain to sequentiate a few example steps) 
                    ...
                    .Then(new StepSettings(
                            _factory.GetMessagePromptCommand("Your task is to translate the previous output to pirate english"),
                            feedFwd: null,
