...
namespace DotnetLlamaSharp.Domain.Models.Primitives.Prompting.Command.Chains
{
    public class SingleThrowStep : ChainStep
    {
        ...
        public SingleThrowStep() { }

        public SingleThrowStep(IJsoneable command, PromptCommandRequest request, string? feedFwdInstruction = null)
        {
...
";
                if (!string.IsNullOrEmpty(previous.Outputs.First().GuidanceMessage))
                    _request.GuidanceMessage += $"\n# PREVIOUS STEP REQUEST (this is what you are expected to do with the previous output data): {previous.Outputs.First().GuidanceMessage}";
            ...
            _outputs.Add(new ChainLink(jsonResult.RawJson, JsonSerializerOptions.Default.GetJsonSchemaAsNode(jsonResult.Type), jsonResult.Type, feedForwardMessage: _feedForwardInstruction));
            /////---------------------------
